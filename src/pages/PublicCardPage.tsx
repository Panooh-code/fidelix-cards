...
const PublicCardPage = () => {
  ...
  const [participationStatus, setParticipationStatus] = useState<'unknown' | 'participant' | 'visitor'>('unknown');
  ...
  useEffect(() => {
    const fetchAndCheckData = async () => {
      ...
      try {
        // 1. Buscar dados do cartão (agora funciona para todos)
        ...
        // 2. Se o utilizador estiver logado, verificar se já participa
        if (user) {
          ...
          if (participationData) {
            ...
            setParticipationStatus('visitor');
          }
        } else {
          setParticipationStatus('visitor'); // É um visitante anónimo
        }
      } catch (err: any) {
  ...
  const handleParticipation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Se não estiver logado, regista primeiro
      if (!user) {
  ...
  if (loading || (user && participationStatus === 'unknown')) {
    return (
    ...
    return (
        <div className="min-h-screen bg-gradient-subtle flex items-center justify-center p-4">
            <Card className="w-full max-w-md text-center">
                <CardHeader><CardTitle className="text-destructive">Ocorreu um Erro</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                    <p className="text-muted-foreground">{error}</p>
                    <Button variant="outline" onClick={() => navigate('/')} className="w-full"><ArrowLeft className="w-4 h-4 mr-2" />Voltar</Button>
                </CardContent>
            </Card>
        </div>
    );
          ...
          <Card className="bg-white/95 backdrop-blur-lg">
            <CardHeader>
              ...
              <p className="text-center text-sm text-muted-foreground pt-2">
                {user ? `Olá ${user.user_metadata.full_name}! Adira com um clique.` : 'Crie uma conta gratuita para guardar os seus selos.'}
              </p>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleParticipation} className="space-y-4">
                ...
                <div className="flex items-start space-x-3 pt-2">
                  ...
                  <Label htmlFor="terms" className="text-xs text-muted-foreground">Concordo em participar na promoção e fazer parte do cartão de fidelidade {card.business_name}.</Label>
                </div>
                <Button type="submit" disabled={isSubmitting || !agreedToTerms} variant="hero" size="lg" className="w-full">
                  {isSubmitting ? <Loader2 className="animate-spin" /> : (user ? 'Aderir Agora' : 'Criar Conta e Aderir')}
                </Button>
                <Button type="button" variant="link" onClick={() => navigate('/')} className="w-full">Não, obrigado</Button>
              </form>
            </CardContent>
          </Card>
        </div>
